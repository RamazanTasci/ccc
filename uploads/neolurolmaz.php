...
if ($action === 'complete_task' && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $res = $conn->query("
        SELECT id, title, sender_id, receiver_id, status 
        FROM tasks 
        WHERE id = $id
    ");
    $t = $res->fetch_assoc();
    ...
    if ($t['status'] === 'completed') {
        ...
        $conn->query("
            UPDATE tasks 
            SET status='completed', completed_by=$me_id, updated_at=NOW()
            WHERE id=$id
        ");
...
if ($action === 'delete_task' && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (!$task_id) json_err('task_id gerekli');

    // Sadece görev sahibi veya alıcı silebilir
    $res = $conn->query("SELECT sender_id, receiver_id FROM tasks WHERE id=$task_id");
    $t = $res->fetch_assoc();
    if (!$t) json_err('Görev bulunamadı');
    if (!($t['sender_id']==$me_id || $t['receiver_id']==$me_id)) json_err('Yetkiniz yok');

    $conn->begin_transaction();
    $conn->query("DELETE FROM task_files WHERE task_id=$task_id");
    $conn->query("DELETE FROM task_comments WHERE task_id=$task_id");
...
<div class="app">
  <button id="hamburgerBtn" style="display:none;font-size:24px;padding:8px;border:none;background:none;cursor:pointer">☰</button>
  <aside class="sidebar">
...
<!-- <div class="nav-item active" data-filter="all" id="nav-all">Tüm Görevler</div> -->
    ...
    <div style="margin-top:auto">
      <div class="hint">Panel - tüm özellikler arka planda etkinleştirilmiştir.</div>
    </div>
  </aside>
...
<form id="taskForm">
  <div style="margin-top:8px">
    <input name="title" placeholder="Başlık" required style="width:100%;padding:8px;border:1px solid #eee;border-radius:6px">
  </div>
  ...
  <div style="display:flex;gap:8px;margin-top:8px">
    <select name="receiver_id" id="taskReceiver">
  ...
  <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:12px">
    <button type="button" id="taskCancel">İptal</button>
...
if (res.success) {
  const taskId = res.id;
const hasImage = t.file_path ? 'style="color:red;cursor:pointer"' : '';
...
function deleteTask(id) {
  ...
  api('delete_task', {task_id:id}, 'POST').then(res=>{
    if (res.success) loadTasks(currentFilter);
    else alert(res.message || 'Hata');
  });
}
